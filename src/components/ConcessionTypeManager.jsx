...
const ConcessionTypeManager = () => {
  ...
  const fetchConcessionTypes = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`${appConfig.API_PREFIX_V1}/fees/concessions-management/concession-types/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedConcessionType) {
        await axiosInstance.put(`${appConfig.API_PREFIX_V1}/fees/concessions-management/concession-types/${selectedConcessionType.id}`, concessionData);
        setAlert({ open: true, message: 'Concession Type updated successfully!', severity: 'success' });
      } else {
        await axiosInstance.post(`${appConfig.API_PREFIX_V1}/fees/concessions-management/concession-types/`, concessionData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this concession type?')) {
      try {
        await axiosInstance.delete(`${appConfig.API_PREFIX_V1}/fees/concessions-management/concession-types/${id}`);
      ...
      complete: async (results) => {
        try {
          ...
          await axiosInstance.post(
            `${appConfig.API_PREFIX_V1}/fees/concessions-management/concession-types/bulk`,
            validRows
          );
