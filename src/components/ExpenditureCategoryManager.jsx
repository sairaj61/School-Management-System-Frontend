...
const ExpenditureCategoryManager = () => {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`${appConfig.API_PREFIX_V1}/administrative/expenditures/categories/`);
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    try {
      if (editingCategory) {
        ...
        await axiosInstance.put(`${appConfig.API_PREFIX_V1}/administrative/expenditures/categories/${editingCategory.id}`, values);
        toast.success('Category updated successfully!');
      } else {
        // Create new category
        await axiosInstance.post(`${appConfig.API_PREFIX_V1}/administrative/expenditures/categories/`, values);
  ...
  const handleDelete = async (categoryId) => {
    if (window.confirm('Are you sure you want to delete this expenditure category?')) {
      try {
        await axiosInstance.delete(`${appConfig.API_PREFIX_V1}/administrative/expenditures/categories/${categoryId}`);
