...
const DriverManager = () => {
  ...
  const fetchDrivers = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`${appConfig.API_PREFIX_V1}/academic/transport`);
  ...
  const fetchStudentsByDriver = async (driverId) => {
    ...
    try {
      const response = await axiosInstance.get(`${appConfig.API_PREFIX_V1}/academic/transport/${driverId}/students`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedDriver) {
        await axiosInstance.put(`${appConfig.API_PREFIX_V1}/academic/transport/${selectedDriver.id}`, driverData);
        setAlert({ open: true, message: 'Driver updated successfully!', severity: 'success' });
      } else {
        await axiosInstance.post(`${appConfig.API_PREFIX_V1}/academic/transport/`, driverData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this driver?')) {
      try {
        await axiosInstance.delete(`${appConfig.API_PREFIX_V1}/academic/transport/${id}`);
