...
const ExpenditureManager = () => {
  ...
  const fetchDependencies = async () => {
    try {
      const [categoriesRes, academicYearsRes] = await Promise.all([
        axiosInstance.get(`${appConfig.API_PREFIX_V1}/administrative/expenditure-management/expenditures/categories/`),
  ...
  const fetchExpenditures = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`${appConfig.API_PREFIX_V1}/administrative/expenditure-management/expenditures/`);
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    try {
      if (editingExpenditure) {
        ...
        await axiosInstance.put(`${appConfig.API_PREFIX_V1}/administrative/expenditure-management/expenditures/${editingExpenditure.id}`, values);
        toast.success('Expenditure updated successfully!');
      } else {
        // Create new expenditure
        await axiosInstance.post(`${appConfig.API_PREFIX_V1}/administrative/expenditure-management/expenditures/`, values);
  ...
  const handleDelete = async (expenditureId) => {
    if (window.confirm('Are you sure you want to delete this expenditure?')) {
      try {
        await axiosInstance.delete(`${appConfig.API_PREFIX_V1}/administrative/expenditure-management/expenditures/${expenditureId}`);
