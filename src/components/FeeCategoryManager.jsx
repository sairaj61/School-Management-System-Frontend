...
const FeeCategoryManager = () => {
  ...
  const fetchFeeCategories = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`${appConfig.API_PREFIX_V1}/fees/fees-management/fee-categories/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedCategory) {
        await axiosInstance.put(`${appConfig.API_PREFIX_V1}/fees/fees-management/fee-categories/${selectedCategory.id}`, categoryData);
        setAlert({ open: true, message: 'Fee Category updated successfully!', severity: 'success' });
      } else {
        await axiosInstance.post(`${appConfig.API_PREFIX_V1}/fees/fees-management/fee-categories/`, categoryData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this fee category?')) {
      try {
        await axiosInstance.delete(`${appConfig.API_PREFIX_V1}/fees/fees-management/fee-categories/${id}`);
      ...
      complete: async (results) => {
        try {
          ...
          await axiosInstance.post(
            `${appConfig.API_PREFIX_V1}/fees/fees-management/fee-categories/bulk`,
            validRows
          );
