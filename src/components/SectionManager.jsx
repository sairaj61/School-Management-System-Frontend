...
const SectionManager = () => {
  ...
  const fetchSections = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`${appConfig.API_PREFIX_V1}/students-managements/sections/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedSection) {
        ...
        await axiosInstance.put(`${appConfig.API_PREFIX_V1}/students-managements/sections/${selectedSection.id}`, sectionData);
        setAlert({ open: true, message: 'Section updated successfully!', severity: 'success' });
      } else {
        // API for POST request uses 'name' and 'class_id'
        await axiosInstance.post(`${appConfig.API_PREFIX_V1}/students-managements/sections/`, sectionData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this section?')) {
      try {
        await axiosInstance.delete(`${appConfig.API_PREFIX_V1}/students-managements/sections/${id}`);
