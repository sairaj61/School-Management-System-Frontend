...
const AutoManager = () => {
  ...
  const fetchAutos = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`${appConfig.API_PREFIX_V1}/students/transport/autos`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedAuto) {
        await axiosInstance.put(`${appConfig.API_PREFIX_V1}/students/transport/autos/${selectedAuto.id}`, formData);
        setAlert({ open: true, message: 'Auto updated successfully!', severity: 'success' });
      } else {
        await axiosInstance.post(`${appConfig.API_PREFIX_V1}/students/transport/autos`, formData);
  ...
  const handleAssignSubmit = async () => {
    try {
      await axiosInstance.post(
        `${appConfig.API_PREFIX_V1}/students/transport/autos/${selectedAuto.id}/assign-students`, 
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this auto? This will remove all student assignments.')) {
      try {
        setLoading(true);
        await axiosInstance.delete(`${appConfig.API_PREFIX_V1}/students/transport/autos/${id}`);
        setAlert({ 
          open: true, 
